...
- ⬜ next-intl setup and migration of English strings from TranslatePress. **URL structure locked** (2026-06-16) by [ADR 0005](decisions/0005-i18n-url-structure.md): path prefix with BG as the unprefixed default — BG at `/`, `/about-us/`, etc., EN at `/en/`, `/en/about-us/`, etc. Matches the live WP/TranslatePress site exactly, so every Google-indexed URL keeps resolving 1:1 after the Stage 6 DNS switch. Decision is non-negotiable; see ADR for the alternatives considered and why each was rejected. The implementation slice covers: `next-intl` middleware + config, `[locale]` route shape (or middleware-only), message-file structure (`messages/bg.json` + `messages/en.json`), migrating every hardcoded BG string in the section components to keys, wiring the LanguageSwitcher to actually swap locales, doubling the sitemap with hreflang pairs, locale-aware OG metadata, and deciding the policy for blog posts that only have BG content (show with notice vs. filter from EN list).
