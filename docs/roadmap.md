...
- 🔄 Contact-form abuse defenses — **honeypot shipped with the form** (offscreen `name="website"` field; server action silently succeeds on fill). **Rate limiter wired** (2026-06-16): `@upstash/ratelimit` + `@upstash/redis` sliding window, 5 submissions per IP per hour, runs after the honeypot and before validation/SMTP so invalid payloads still throttle. IP read from `x-forwarded-for` (first hop) with `x-real-ip` fallback. Fails open with a console warning if env vars are missing, so local dev works without an Upstash account. **Still pending before public launch**: provision the Upstash free-tier Redis and set `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` on Vercel (Production + Preview + Development) — see follow-ups. Optional Cloudflare Turnstile for residual spam.
...
- **Upstash Redis not yet provisioned.** The contact-form rate limiter (`src/lib/rateLimit.ts`) ships fail-open: missing env vars = limit disabled with a console warning. Before public launch, create a free-tier Upstash Redis (region close to Vercel's `fra1`/`cdg1` for lowest latency), then set `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` on Vercel Production, Preview, and Development. After setting them, pull locally with `vercel env pull .env.local` so the dev server picks them up.